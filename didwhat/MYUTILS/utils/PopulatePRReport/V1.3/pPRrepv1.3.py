...
workbook_ExtPkgs = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\ExtendedPackages_wSuppInfo.xlsx'
#workbook_ExtPkgs = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\ExtendedPackages_wSuppInfo_Test.xlsx'
#workbook_PR_rep = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\PRTracking.xlsx'
workbook_PR_rep = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\PRTracking_Test.xlsx'

#workbook_Consolidated_rep = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\Consolidated_PackageUpgradeReport.xlsx'
workbook_Consolidated_rep = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\Consolidated_PackageUpgradeReport_Test.xlsx'

#workbook_Csld_w_BB = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\BulkBuild\\Consolidated_26Nov_w_BulkColumns.xlsx'
workbook_Csld_w_BB = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\BulkBuild\\Consolidated_26Nov_w_BulkColumns_Test.xlsx'

#workbook_BB_Slist = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\BulkBuild\\bulkbuilt_success_list.xlsx'
workbook_BB_Slist = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\BulkBuild\\bulkbuilt_2nd_success_list.xlsx'

workbook_already_anlzd = "C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\AutoAnalyzed\\Analysis_Result_BeingUsed_06Nov\\all-1563-pkg-list1.xlsx"
...
def cleanup_xl_sheet():
    """ Remove spaces from around pkg names
        ...
        Delete empty rows
    """
    wb = openpyxl.load_workbook(workbook_ExtPkgs)  
    ws = wb.active
...
#            print (f'After the split the package is {pkg}')
            #rnumber 1 of the table, i.e. rnum = 1, corresponds to row number 3 of the 
            #worksheet.  Hence we do rnum + 2.
            ws.cell(row=rnum+2, column=1).value = pkg[0]
    wb.save(workbook_ExtPkgs)


def pick_evaluator():
    """ Pick the evaluator column
    """
    wb_ExtPkgs = openpyxl.load_workbook(workbook_ExtPkgs)  
    ws_ExtPkgs = wb_ExtPkgs.active

    wb_PR_rep = openpyxl.load_workbook(workbook_PR_rep)  
    ws_PR_rep = wb_PR_rep.active

    wb_CSLDTD_rep = openpyxl.load_workbook(workbook_Consolidated_rep)  
    ws_CSLDTD_rep = wb_CSLDTD_rep.active

    wb_CSLDTD_wBB_rep = openpyxl.load_workbook(workbook_Csld_w_BB)  
    ws_CSLDTD_wBB_rep = wb_CSLDTD_wBB_rep.active

    wb_BB_Slist = openpyxl.load_workbook(workbook_BB_Slist)  
    ws_BB_Slist = wb_BB_Slist.active

    wb_alrdy_anlzd = openpyxl.load_workbook(workbook_already_anlzd)  
    ws_alrdy_anlzd = wb_alrdy_anlzd.active

    workbook_Akarsh = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\PackageUpgradeReport-Akarsh.xlsx'
#    workbook_Akarsh = 'C:\\Users\\v-nsallh\\Utils\\PopulatePRReport\\PackageUpgradeReport-Akarsh.xlsx'
    wb_Akarsh = openpyxl.load_workbook(workbook_Akarsh)  
    ws_Akarsh = wb_Akarsh.active

    workbook_Aninda = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\PackageUpgradeReport-Aninda.xlsx'
    wb_Aninda = openpyxl.load_workbook(workbook_Aninda)  
    ws_Aninda = wb_Aninda.active

    workbook_Jagadeesh = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\PackageUpgradeReport-Jagadeesh.xlsx'
    wb_Jagadeesh = openpyxl.load_workbook(workbook_Jagadeesh)  
    ws_Jagadeesh = wb_Jagadeesh.active

    workbook_Jyoti = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\PackageUpgradeReport-Jyoti.xlsx'
    wb_Jyoti = openpyxl.load_workbook(workbook_Jyoti)  
    ws_Jyoti = wb_Jyoti.active

    workbook_Kevin = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\PackageUpgradeReport-Kevin.xlsx'
    wb_Kevin = openpyxl.load_workbook(workbook_Kevin)  
    ws_Kevin = wb_Kevin.active

    workbook_Neeraj = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\PackageUpgradeReport-Neeraj.xlsx'
    wb_Neeraj = openpyxl.load_workbook(workbook_Neeraj)  
    ws_Neeraj = wb_Neeraj.active

    workbook_Sreenivas = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\PackageUpgradeReport-Sreenivas.xlsx'
    wb_Sreenivas = openpyxl.load_workbook(workbook_Sreenivas)  
    ws_Sreenivas = wb_Sreenivas.active

    workbook_Sumit = 'C:\\Users\\v-nsallh\\OneDrive - Microsoft\\Documents\\Work\\PackageUpgrades-3.0\\PackageUpgradeReport-Sumit.xlsx'
    wb_Sumit = openpyxl.load_workbook(workbook_Sumit)  
    ws_Sumit = wb_Sumit.active

    rnum = 0
    for row in ws_ExtPkgs.iter_rows(min_row=3, values_only=True): #min_row=3 as pkg names start from row 3        
        rnum = rnum + 1
#        print (f'row is {row}, row[0] is {row[0]}, row[1] is {row[1]}, row[2] is {row[2]}')
        pkgname = row[0]
        evaluator = row[1]

        if evaluator == f'Akarsh': 
#            print(f'The file we will pick is PackageUpgradeReport-Akarsh.xlsx')
            ws_forimportinginfo = ws_Akarsh
        elif evaluator == f'Aninda': 
#            print(f'The file we will pick is PackageUpgradeReport-Aninda.xlsx')
            ws_forimportinginfo = ws_Aninda
        elif evaluator == f'Jagadeesh': 
#            print(f'The file we will pick is PackageUpgradeReport-Jagadeesh.xlsx')
            ws_forimportinginfo = ws_Jagadeesh
        elif evaluator == f'Jyoti':
#            print(f'The file we will pick is PackageUpgradeReport-Jyoti.xlsx')
            ws_forimportinginfo = ws_Jyoti
        elif evaluator == f'Kevin': 
#            print(f'The file we will pick is PackageUpgradeReport-Kevin.xlsx')
            ws_forimportinginfo = ws_Kevin
        elif evaluator == f'Neeraj': 
#            print(f'The file we will pick is PackageUpgradeReport-Neeraj.xlsx')
            ws_forimportinginfo = ws_Neeraj
        elif evaluator == f'Sreenivasulu': 
#            print(f'The file we will pick is PackageUpgradeReport-Sreenivas.xlsx')
            ws_forimportinginfo = ws_Sreenivas
        elif evaluator == f'Sumit': 
#            print(f'The file we will pick is PackageUpgradeReport-Sumit.xlsx')
            ws_forimportinginfo = ws_Sumit
            

        anlzd_rnum = 0
        print (f'Finding the versions of package {pkgname} in 3.0 and 2.0')
        found_in_analyzed_ws = 0
        for row_in_anlzd in ws_alrdy_anlzd.iter_rows(min_row=3, values_only=True): #min_row=3 as pkg names start from row 3        
            anlzd_rnum = anlzd_rnum + 1
            pkgnm_from_anlzd = row_in_anlzd[2]
#            print (f'row_in_anlzd is "{row_in_anlzd}", pkgnm_from_anlzd is {pkgnm_from_anlzd}, pkgname is {pkgname}')
#            print (f'row_in_anlzd[0] is {row_in_anlzd[0]}, row_in_anlzd[1] is {row_in_anlzd[1]}, row_in_anlzd[2] is {row_in_anlzd[2]}, row_in_anlzd[14] is {row_in_anlzd[14]}, row_in_anlzd[15] is {row_in_anlzd[15]}') 
            ver_in_2 = f'Not_Found'
            ver_in_3 = f'Not_Found'
            if ( pkgnm_from_anlzd == pkgname ):
                print (f'Match found for {pkgname} at anlzd_rnum = {anlzd_rnum}')
                ver_in_2 = row_in_anlzd[14]
                ver_in_3 = row_in_anlzd[15]

                #lets populate the Consolidate Package Upgrade activity sheet from the already analyzed results
                #later on if an entry is found in the per developer sheets the row will be overwritten with those values
                ws_CSLDTD_rep.cell(row=rnum+2, column=2).value=rnum;
                ws_CSLDTD_rep.cell(row=rnum+2, column=3).value=pkgname;
                ws_CSLDTD_rep.cell(row=rnum+2, column=4).value=row_in_anlzd[3];
                ws_CSLDTD_rep.cell(row=rnum+2, column=5).value=row_in_anlzd[4];
                ws_CSLDTD_rep.cell(row=rnum+2, column=6).value=row_in_anlzd[5];
                ws_CSLDTD_rep.cell(row=rnum+2, column=7).value=row_in_anlzd[6];
                ws_CSLDTD_rep.cell(row=rnum+2, column=8).value=f'Not Started';
                ws_CSLDTD_rep.cell(row=rnum+2, column=9).value=f'NOT_FOUND';
                ws_CSLDTD_rep.cell(row=rnum+2, column=10).value=row_in_anlzd[9];
                ws_CSLDTD_rep.cell(row=rnum+2, column=11).value=row_in_anlzd[10];
                ws_CSLDTD_rep.cell(row=rnum+2, column=12).value=row_in_anlzd[11];
                ws_CSLDTD_rep.cell(row=rnum+2, column=13).value=row_in_anlzd[12];
                ws_CSLDTD_rep.cell(row=rnum+2, column=14).value=row_in_anlzd[13];
                ws_CSLDTD_rep.cell(row=rnum+2, column=15).value=ver_in_2;
                ws_CSLDTD_rep.cell(row=rnum+2, column=16).value=ver_in_3;
                ws_CSLDTD_rep.cell(row=rnum+2, column=17).value=row_in_anlzd[16];
                ws_CSLDTD_rep.cell(row=rnum+2, column=18).value=row_in_anlzd[17];
                ws_CSLDTD_rep.cell(row=rnum+2, column=19).value=row_in_anlzd[18];
                ws_CSLDTD_rep.cell(row=rnum+2, column=20).value=row_in_anlzd[19];
                ws_CSLDTD_rep.cell(row=rnum+2, column=21).value=row_in_anlzd[20];
    
                #lets populate the duplicate fields of Consolidate Package with Bulk build info sheet from the already analyzed results
                #later on if an entry is found in the per developer sheets the row will be overwritten with those values
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=2).value=rnum;
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=3).value=pkgname;
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=4).value=row_in_anlzd[3];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=5).value=row_in_anlzd[4];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=6).value=row_in_anlzd[5];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=7).value=row_in_anlzd[6];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=8).value=f'Not Started';
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=9).value=f'NOT_FOUND';
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=10).value=row_in_anlzd[9];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=11).value=row_in_anlzd[10];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=12).value=row_in_anlzd[11];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=13).value=row_in_anlzd[12];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=14).value=row_in_anlzd[13];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=15).value=ver_in_2;
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=16).value=ver_in_3;
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=17).value=row_in_anlzd[16];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=18).value=row_in_anlzd[17];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=19).value=row_in_anlzd[18];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=20).value=row_in_anlzd[19];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=21).value=row_in_anlzd[20];

                break;

        importws_rnum = 0
        print (f'Looking for package name = {pkgname} in the developers file')
        found_flag = 0
        for row in ws_forimportinginfo.iter_rows(min_row=3, values_only=True): #min_row=3 as pkg names start from row 3        
            importws_rnum = importws_rnum + 1
            pkgnm_fromimp = row[2]
#            evaluator_fromimp = row[6]
            prlink_fromimp = row[8]
            if ( pkgnm_fromimp == pkgname ):
                found_flag = 1
                print (f'Found {pkgname} in excel sheet of evaluator {evaluator} at row {importws_rnum}')
#                print (f'package name {pkgname} found')
#                print (f' row num = {importws_rnum}, package = {pkgnm_fromimp} evaluator = {evaluator_fromimp} PR = {prlink_fromimp}')
                #lets populate the PR report worksheet for serial no, package name, developer, PR Link
                #note that the row number are the same in the Extended package worksheet and the PR link work sheet
                #in both the row start from 3rd row.  row=rnum+2 is used for that.
                #Serial number
                ws_PR_rep.cell(row=rnum+2, column=2).value=rnum;
                ws_PR_rep.cell(row=rnum+2, column=3).value=pkgname;
                ws_PR_rep.cell(row=rnum+2, column=4).value=evaluator;
                ws_PR_rep.cell(row=rnum+2, column=5).value=prlink_fromimp;

                #if the developer has marked it as Revisit or has started work on a package
                #then the decision of the developer will reflect in the "Need Upgrade" column
                #otherwise, the decision made during automated analysis is used
                need_upg_flag = row_in_anlzd[3];
                if ( row[3] == f'Revisit'):
                    need_upg_flag = f'Revisit'
                else:
                    if ( row[7] != f'Not_Started' and row[7] != f'Not Started' ):
                        need_upg_flag = row[3]    

                #lets populate the Consolidate Package Upgrade activity sheet
                ws_CSLDTD_rep.cell(row=rnum+2, column=2).value=rnum;
                ws_CSLDTD_rep.cell(row=rnum+2, column=3).value=pkgname;
                ws_CSLDTD_rep.cell(row=rnum+2, column=4).value=need_upg_flag;
                ws_CSLDTD_rep.cell(row=rnum+2, column=5).value=row[4];
                ws_CSLDTD_rep.cell(row=rnum+2, column=6).value=row[5];
                ws_CSLDTD_rep.cell(row=rnum+2, column=7).value=row[6];
                ws_CSLDTD_rep.cell(row=rnum+2, column=8).value=row[7];
                ws_CSLDTD_rep.cell(row=rnum+2, column=9).value=row[8];
                ws_CSLDTD_rep.cell(row=rnum+2, column=10).value=row[9];
                ws_CSLDTD_rep.cell(row=rnum+2, column=11).value=row[10];
                ws_CSLDTD_rep.cell(row=rnum+2, column=12).value=row[11];
                ws_CSLDTD_rep.cell(row=rnum+2, column=13).value=row[12];
                ws_CSLDTD_rep.cell(row=rnum+2, column=14).value=row[13];
                ws_CSLDTD_rep.cell(row=rnum+2, column=15).value=ver_in_2;
                ws_CSLDTD_rep.cell(row=rnum+2, column=16).value=ver_in_3;
                ws_CSLDTD_rep.cell(row=rnum+2, column=17).value=row[16];
                ws_CSLDTD_rep.cell(row=rnum+2, column=18).value=row[17];
                ws_CSLDTD_rep.cell(row=rnum+2, column=19).value=row[18];
                ws_CSLDTD_rep.cell(row=rnum+2, column=20).value=row[19];
                ws_CSLDTD_rep.cell(row=rnum+2, column=21).value=row[20];

                #lets populate the duplicate fields of Consolidate Package with Bulk build info sheet
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=2).value=rnum;
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=3).value=pkgname;
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=4).value=need_upg_flag;
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=5).value=row[4];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=6).value=row[5];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=7).value=row[6];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=8).value=row[7];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=9).value=row[8];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=10).value=row[9];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=11).value=row[10];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=12).value=row[11];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=13).value=row[12];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=14).value=row[13];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=15).value=ver_in_2;
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=16).value=ver_in_3;
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=17).value=row[16];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=18).value=row[17];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=19).value=row[18];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=20).value=row[19];
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=21).value=row[20];

        if ( found_flag != 1):
            #lets populate the PR report worksheet for serial no, package name, developer.  The PR Link will be marked 
            # NOT FOUND to indicate the inconsistency that the entry was not found in the individual developer's 
            #excel work book
            #
            #No need to update the consolidated data sheets in case the entry is not found in developers excel sheet
            #in that case we will use the data already populated using the automated analysis
            print (f'package name {pkgname} missing')
            ws_PR_rep.cell(row=rnum+2, column=2).value=rnum;
            ws_PR_rep.cell(row=rnum+2, column=3).value=pkgname;
            ws_PR_rep.cell(row=rnum+2, column=4).value=evaluator;
            ws_PR_rep.cell(row=rnum+2, column=5).value=f'NOT_FOUND';


        Sbulk_list_rnum = 0
        found_inbblist = 0
        for bblist_row in ws_BB_Slist.iter_rows(min_row=1, values_only=True):
            Sbulk_list_rnum = Sbulk_list_rnum + 1
            pkgnm_from_bblist = bblist_row[0]
            pkgvr_from_bblist = bblist_row[1]

            #print (f'Row Sbulk_list_rnum {Sbulk_list_rnum}, bblist_row is {bblist_row}, bblist_row[0] is {bblist_row[0]}, bblist_row[1] is {bblist_row[1]}')

            #lets populate the Consolidate Package with Bulk build info sheet


            if ( pkgnm_from_bblist == pkgname ):
                print (f'Found {pkgname} in Bulk Build success list at row {Sbulk_list_rnum}')
                found_inbblist = 1
                #the package name again
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=22).value=pkgnm_from_bblist;
                #the version in Bulk build
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=23).value=pkgvr_from_bblist;
                #set the bulk build status success flag to Y
                ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=24).value= f'Y';
                #Does the bulk build version of package match the version in 3.0-dev
#                print (f'3.0 ver and build ver check: {str(ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=16).value)}, {pkgvr_from_bblist}')

                if ( str(ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=16).value) == pkgvr_from_bblist):
                    ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=25).value= f'Y';
                else:
                    ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=25).value= f'N';


        if (found_inbblist == 0):
            print (f'Missing {pkgname} in Bulk Build success list')
            #set the bulk build status success flag to N
            ws_CSLDTD_wBB_rep.cell(row=rnum+2, column=24).value= f'N';


    #we can write to the three files in the very end after having prepared the things
    #in memory (ws_PR_rep and ws_CSLDTD_rep)
    #that's why we are doing it outside the for loops
    wb_PR_rep.save(workbook_PR_rep)
    wb_CSLDTD_rep.save(workbook_Consolidated_rep)
    wb_CSLDTD_wBB_rep.save(workbook_Csld_w_BB)




#Neeraj            pkg = row[2].strip(' \t\n\r')
#Neeraj            # pkg_list.append(pkg) # row[2] = our col 'C'
#Neeraj            pkg = re.split(r"-(?=\d)", pkg)
#Neeraj            ws.cell(row=row[1]+2, column=3).value = pkg[0]
#Neeraj    wb.save(workbook)
...
#Neeraj    print('Please wait for the app to load ....')
#Neeraj    get_file_paths_from_user()
#Neeraj    # pkg = 'kyotocabinet'
#Neeraj    # version = '1.2.80'
#Neeraj    # release = '6.fc41'
#Neeraj    # url = f'https://kojipkgs.fedoraproject.org//packages/{pkg}/{version}/{release}/src/{pkg}-{version}-{release}.src.rpm'
#Neeraj    # urllib.request.urlretrieve(url, f'{pkg}.src.rpm')
#Neeraj    # extract_src_rpm(f'{pkg}.src.rpm', f'{pkg}')
#Neeraj    print('Now Processing ........Please wait')
#Neeraj    print('For more info you may refer the log file: pkg_update_analysis.log')
    cleanup_xl_sheet()
    print('Now Processing: read_all_pkg_names. Please wait.........')
    logger.info('Now Processing: read_all_pkg_names. Please wait.........')
    pick_evaluator()
#Neeraj    read_all_pkg_names()
#Neeraj    print('Now Processing: update_daily_build_status. Please wait.........')
#Neeraj    logger.info('Now Processing: update_daily_build_status. Please wait.........')
#Neeraj    update_daily_build_status()
#Neeraj    print('Now Processing: update_current_pkg_versions. Please wait.........')
#Neeraj    logger.info('Now Processing: update_current_pkg_versions. Please wait.........')
#Neeraj    update_current_pkg_versions()
#Neeraj    print('Now Processing: update_latest_fedora_pkg_info. Please wait.........')
#Neeraj    logger.info('Now Processing: update_latest_fedora_pkg_info. Please wait.........')
#Neeraj    update_latest_fedora_pkg_info()
#Neeraj    print('Now Processing: update_if_need_upgrade. Please wait.........')
#Neeraj    logger.info('Now Processing: update_if_need_upgrade. Please wait.........')
#Neeraj    update_if_need_upgrade()
#Neeraj    print('Now Processing: update_pkg_status. Please wait.........')
#Neeraj    logger.info('Now Processing: update_pkg_status. Please wait.........')
#Neeraj    update_pkg_status()
#Neeraj    print('All Processing: DONE')
#Neeraj    logger.info('All Processing: DONE')
#Neeraj    while True:
#Neeraj        user_input = input("Type 'x' to quit): ")
#Neeraj        if user_input == "x":
#Neeraj            sys.exit("Exiting program...")
#Neeraj        else:
#Neeraj            print("You entered:", user_input)
